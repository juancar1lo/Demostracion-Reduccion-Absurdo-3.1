...
function parseFormula(input) {
    ...
    if (index < input.length) {
        throw new Error("Error de parseo en: " + input.substr(index));
    }
    return result;
}
...
function completeBranch(branch, missingVars) {
    ...
    return completeBranch(branchTrue, rest).concat(completeBranch(branchFalse, rest));
}
...
function checkContradiction(branch) {
    let seen = {};
    for (let entry of branch.entries) {
        if (entry.formula.type === 'var') {
...
function expandBranch(branch) {
    let contr = checkContradiction(branch);
    if (contr.contradiction) {
        branch.closed = true;
        branch.steps.push("Contradicción encontrada en la variable <span class='highlight'>" + contr.variable + "</span>.");
        return [branch];
    }
    let indexToExpand = -1;
    ...
    switch (entry.formula.type) {
        case 'not':
            ruleDescription = "Aplicando regla de negación en " + displayFormula(entry.formula);
            ...
            branchCopy.steps.push(ruleDescription + ": se asigna " + (newEntry.value ? "VERDADERO" : "FALSO") + " a " + displayFormula(entry.formula.sub));
            newBranches = expandBranch(branchCopy);
            break;
        case 'and':
            if (entry.value === true) {
                ruleDescription = "Descomponiendo conjunción verdadera en " + displayFormula(entry.formula);
                let branchCopy = cloneBranch(branch);
                ...
                branchCopy.steps.push(ruleDescription + ": se asigna VERDADERO a " + displayFormula(entry.formula.left) + " y " + displayFormula(entry.formula.right));
                newBranches = expandBranch(branchCopy);
            } else {
                ruleDescription = "Descomponiendo conjunción falsa en " + displayFormula(entry.formula);
                let branchCopy1 = cloneBranch(branch);
                branchCopy1.entries.push({ formula: entry.formula.left, value: false, processed: false });
                branchCopy1.steps.push(ruleDescription + " (rama 1): se asigna FALSO a " + displayFormula(entry.formula.left));
                let branchCopy2 = cloneBranch(branch);
                branchCopy2.entries.push({ formula: entry.formula.right, value: false, processed: false });
                branchCopy2.steps.push(ruleDescription + " (rama 2): se asigna FALSO a " + displayFormula(entry.formula.right));
                newBranches = expandBranch(branchCopy1).concat(expandBranch(branchCopy2));
            }
            break;
        case 'or':
            if (entry.value === true) {
                if (isComplementary(entry.formula.left, entry.formula.right)) {
                    ruleDescription = "Tautología detectada en " + displayFormula(entry.formula) + ". Se marca como procesada sin expandir.";
                    branch.steps.push(ruleDescription);
                    newBranches = expandBranch(branch);
                } else {
                    ruleDescription = "Descomponiendo disyunción verdadera en " + displayFormula(entry.formula);
                    let branchCopy1 = cloneBranch(branch);
                    branchCopy1.entries.push({ formula: entry.formula.left, value: true, processed: false });
                    branchCopy1.steps.push(ruleDescription + " (rama 1): se asigna VERDADERO a " + displayFormula(entry.formula.left));
                    let branchCopy2 = cloneBranch(branch);
                    branchCopy2.entries.push({ formula: entry.formula.right, value: true, processed: false });
                    branchCopy2.steps.push(ruleDescription + " (rama 2): se asigna VERDADERO a " + displayFormula(entry.formula.right));
                    newBranches = expandBranch(branchCopy1).concat(expandBranch(branchCopy2));
                }
            } else {
                ruleDescription = "Descomponiendo disyunción falsa en " + displayFormula(entry.formula);
                let branchCopy = cloneBranch(branch);
                ...
                branchCopy.steps.push(ruleDescription + ": se asigna FALSO a " + displayFormula(entry.formula.left) + " y " + displayFormula(entry.formula.right));
                newBranches = expandBranch(branchCopy);
            }
            break;
        case 'conditional':
            if (entry.value === true) {
                ruleDescription = "Descomponiendo condicional verdadero en " + displayFormula(entry.formula);
                let branchCopy1 = cloneBranch(branch);
                branchCopy1.entries.push({ formula: entry.formula.left, value: false, processed: false });
                branchCopy1.steps.push(ruleDescription + " (rama 1): se asigna FALSO a " + displayFormula(entry.formula.left));
                let branchCopy2 = cloneBranch(branch);
                ...
                ruleDescription = "Descomponiendo condicional falso en " + displayFormula(entry.formula);
                let branchCopy = cloneBranch(branch);
                branchCopy.entries.push({ formula: entry.formula.left, value: true, processed: false });
                branchCopy.entries.push({ formula: entry.formula.right, value: false, processed: false });
                branchCopy.steps.push(ruleDescription + ": se asigna VERDADERO a " + displayFormula(entry.formula.left) + " y FALSO a " + displayFormula(entry.formula.right));
                newBranches = expandBranch(branchCopy);
            }
            break;
        case 'biconditional':
            if (entry.value === true) {
                ruleDescription = "Reemplazando bicondicional VERDADERO en " + displayFormula(entry.formula) +
                ...
                ruleDescription = "Reemplazando bicondicional FALSO en " + displayFormula(entry.formula) +
                    " por (( " + displayFormula(entry.formula.left) + " ∧ ¬" + displayFormula(entry.formula.right) + ") ∨ (¬" +
...
function completeOpenBranches(branches, ast) {
    ...
    for (let branch of branches) {
        ...
        branch.entries.forEach(entry => {
            if (entry.formula.type === 'var') {
                assigned[entry.formula.name] = entry.value;
            }
        });
        let assignedVars = Object.keys(assigned);
        let missing = allVars.filter(v => !assignedVars.includes(v));
        let completeBranches = completeBranch(branch, missing);
        ...
        if (!counterexampleFound) {
            branch.closed = true;
            branch.steps.push("Tras completar las asignaciones, no se encontró contraejemplo.");
            completed.push(branch);
        }
    }
    return completed;
...
function displayBranch(branch, index) {
...
function displayResults(branches, allClosed) {
    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = "";

    const header = document.createElement("h2");
    header.textContent = allClosed ?
        "La fórmula es VÁLIDA (reducción al absurdo exitosa)." :
        "La fórmula NO es válida.";
    resultDiv.appendChild(header);

    if (!allClosed) {
        const p = document.createElement("p");
        p.textContent = "Se encontró al menos una rama sin contradicción (contraejemplo):";
    ...
    branches.forEach((branch, index) => {
        if (branch.closed || (!branch.closed && !allClosed)) {
            setTimeout(() => {
                const branchElement = displayBranch(branch, index);
...
function solveFormula() {
    ...
    let initialBranch = {
        ...
        steps: ["Suponemos que la fórmula " + displayFormula(ast) + " es FALSA."],
        closed: false
    };
    let branches = expandBranch(initialBranch);
    branches = completeOpenBranches(branches, ast);
    let allClosed = branches.every(branch => branch.closed);

    displayResults(branches, allClosed);
}